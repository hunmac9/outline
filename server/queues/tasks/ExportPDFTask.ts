...
import { Attachment, Document, FileOperation, Collection, User, Team } from "@server/models";
...
export default class ExportPDFTask extends BaseTask<ExportPDFTaskPayload> {
  ...
  public async perform(payload: ExportPDFTaskPayload) {
    ...
    const includeAttachments = fileOperation.options?.includeAttachments ?? true;
    ...
    try {
      ...
      Logger.info("task", `Exporting ${documents.length} documents as PDF`, { fileOperationId });
      ...
      for (const document of documents) {
        const documentPath = pathMap.get(`/doc/${document.id}`) || `${serializeFilename(document.title)}.pdf`;
        Logger.info("task", `Generating PDF for document: ${document.id}`, { fileOperationId, documentPath });
        ...
        try {
          ...
          await page.waitForFunction(() => (window as any).status === "ready", { timeout: 30000 });
        } catch (err) {
           Logger.warn(`Timeout or error waiting for Mermaid rendering signal for document ${document.id}`, { 
             error: err,
             fileOperationId 
           });
        }
        ...
        if (includeAttachments && node) { // Check if node is not null
      ...
      await new Promise<void>((resolve, reject) => { // Add void type for clarity
      ...
      void fileOperation.update({
         ...
         Logger.error("Failed to update FileOperation to Error state", updateErr, { fileOperationId });
      });
    } finally {
